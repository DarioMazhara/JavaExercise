...
public class Student implements Comparable<Student> { // compares students based on name
    ...
    public String toString() {
        ...
        return "Name: " + this.name + " id: " + this.id; // remember to change this
    }
    ...
    public int compareTo(Student otherStudent) {
        ...
        if (this.name.equals(otherStudent.getName())) {
            compare = this.id.compareTo(otherStudent.getId());
        }
        else {
            compare = this.name.compareTo(otherStudent.getName());
        }
